...
for (i in 1:length(files)){
  ...
  for (j in 0:length(p_vals)){
    if (j == 0){
      ...
      delta_percentage <- combined_data$change_percentage
      ...
      delta_percentage <- combined_data[combined_data$pval < p_vals[j],]$change_percentage
  ...
  percentage_table <- rbind(percentage_table, output)
  
}

colnames(percentage_table) <- c("all (Estimate, Std. Err, Tval, Pval, N)", p_vals)

rownames(percentage_table) <- c("Height", "Cholesterol", "BMI", "Asthma", 
                     "White Blood Cell", "Anxiety", "Depression", "Diabetes", 
                     "Obesity", "Alzheimer's Disease")

write.table(percentage_table, file = "lin_regress_change_percent.tsv", 
